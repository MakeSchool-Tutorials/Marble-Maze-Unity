...
>[action] Now add to Goal a Component called "Goal" that logs a message whenever a Ball collides with it.  As a hint, you may need to modify Ball in some way to make this work.
...
Great!  Remember where you have your log message, because we’re going to come back to it to implement our Scene switch.  For now though, save your current Scene (File->Save Scene or Ctrl-S) and create a new one (File->New Scene or Ctrl-N).

Save this new Scene as "Main" in the Scenes folder.  This will be our Main Menu.

![image alt text](../assets/image_31.png)
...
You may have noticed, by the way, when you created the Canvas that an Event System was added to your Hierarchy as well.  The Event System makes the Button component respond automatically to click and touch inputs.  We won’t be dealing with these now, so you can delete this object if you’d like, but it shouldn’t hinder you to leave it either.
...
![image alt text](../assets/image_34.gif)
...
![image alt text](../assets/image_35.png)
...
To make the text **bolder**, we also set the color to white, and add a few Outline components progressively farther away.

![image alt text](../assets/image_36.png)
...
![image alt text](../assets/image_40.gif)
...
![image alt text](../assets/image_41.png)
...
![image alt text](../assets/image_42.png)

This is because, in Unity, by default, the Skybox’s contribution to a Scene’s lighting is saved temporarily when you’re editing that Scene, so you get to see how it looks when you run the Scene itself.  When you load a Scene from another one though, the Skybox’s contribution to lighting in that scene hasn’t been calculated anywhere, so Unity lights the Scene without accounting for the Skybox’s contribution.

>[action] To remedy this, open up the Play Scene, then open up the Lighting Window, uncheck the "Auto" button at the bottom, and press “Build.”  A folder should be added to the Scenes folder that contains the lighting information for this Scene, so that Unity can use that info to light the scene later.  When we built the lights for Play, we also switched the Skybox to be the same as the one in Main, because we thought it was a little prettier than the default.

![image alt text](../assets/image_43.png)
...
![image alt text](../assets/image_44.png)

![image alt text](../assets/image_45.png)
...
![image alt text](../assets/image_46.png)

![image alt text](../assets/image_47.gif)
